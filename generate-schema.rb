...
describe '#{($catalog[:names][type['name']]||'').split('::').first}' do
  describe '#{($catalog[:names][type['name']]||'').split('::').last}' do
...
'spec/it' => "
    ...
    #  result = response.dig('data', '#{helper['class']}')
    #  expect(result['#{type['name']}']).to eq #{helper['class']}.#{(type['name']||'').underscore}
    #end
",
'spec/type_footer' => "\n  end
end\n",
