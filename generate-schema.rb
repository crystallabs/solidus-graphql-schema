...
$catalog = {
  ...
  # Parts of output, but on a sub-file level. Currently, this is used for filling in
  # queries, responses, and variables visible at the top of generated test files.
  snippets: {},
...
def parse_types_2(v)
  ...
  v.each do |t|
    ...
    $catalog[:spec_contents][new_name].push [template('spec/type_header', t, helper)]
    ...
    if t['fields']
      t['fields'].each do |f|
        ...
        # Granular pieces for test file headers. Do not use any of this for now.
        #$catalog[:snippets][new_name]||= {}
        #$catalog[:snippets][new_name][:queries]||= []
        #$catalog[:snippets][new_name][:queries].push template('spec/field_query', f, helper)
        # Do responses within contexts, not at the top
        #$catalog[:snippets][new_name][:responses]||= []
        #$catalog[:snippets][new_name][:responses].push template('spec/field_response', f, helper)
    ...
    $catalog[:spec_contents][new_name].push template 'spec/type_footer'
  end
end
...
def output_files
  ...
  $catalog[:spec_outputs][name]= 'types/base_interface'
  ...
  $catalog[:spec_outputs][name]= 'types/base_input'
  ...
  # Output total file list (excluding specs):
...
',
'type_header' => "# frozen_string_literal: true\nmodule Spree::GraphQL::#{$catalog[:names][type['name']]}" +
...
'type_footer' => "\nend",
# Test parts:
'spec/type_header' => "# frozen_string_literal: true\nrequire 'spec_helper'
...
'spec/it' => %Q{
    ...
    describe '#{(type['name'] || '')}' do
      let!(:query) { '{ #{helper['class']} { #{type['name']} } }' }
      let!(:result) { { data: { #{helper['class']}: { #{type['name']}: '' }}} }
      #it 'succeeds' do
      #  execute
...
def add_spec_queries(name, contents)
  queries = ( $catalog[:snippets][name] && $catalog[:snippets][name][:queries]) ? $catalog[:snippets][name][:queries] : []
  if queries.size > 0
    q = "\n    let!(:queries) {%q{\n" + queries.map{|x| '      '+ x}.join("\n") + "\n    }}\n"
    contents.insert 1, q
  end
end
